...
REM set "moveCursor=<nul set /p "=!ESC![x;yH""
...
REM (for /l %%i in (1 1 !iMax!) do set "scr= !scr!") & set "emp=!scr!"
...
REM set "aLineBS=" & for /L %%a in (1 1 !wid!) do set "aLineBS=!aLineBS!%BS%"
...
(
	...
	for /L %%i in (0 1 500000) do (
		...
		set /a "th+=-DEG+p2, th %%= p2, th += th>>31&p2, t=th, s1=(t-p#2^t-p3#2)>>31, s3=p3#2_-t>>31, t=(-t&s1)+(t&~s1)+(p&s1)+(pn2&s3), SINt=%SIN%, t=%COS%, COSt=(-t&s1)+(t&~s1), xt=XC-COSt, yt=YC-SINt"
		...
		title %%i / 5000; th=!th!; dth=!dth!;  {hue=!hue!}
	)
)
