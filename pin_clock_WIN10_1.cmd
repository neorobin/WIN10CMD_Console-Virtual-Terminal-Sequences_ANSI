...
set /a "_PI=31416, _2PI=2*_PI, _PI#2=_PI/2, _3PI#2=3*_PI/2, _3PI#2_1=_3PI#2-1, _DEG=_PI/180, _6DEG=6*_PI/180"
...
set /a "_SPEED=1*%_DEG%, th=_TH0+%_2PI%, _DTH=-_SPEED"
...
(
	...
	for /L %%i in (0 1 500000) do (
		...
		set "$pin=!$pin!%_ESC%[38;5;!hue!m"
		for /l %%a in (0 1 %_HALF_WID%) do (
			set /a "#x=($x+=#C)/10000+1, #y=($y+=#S)/10000+1"
			REM set "$pin=!$pin!%_ESC%[!#x!;!#y!H%_ESC%[38;5;!hue!m@"
			set "$pin=!$pin!%_ESC%[!#x!;!#y!H@"
		)
		...
		REM title %%i / 500000; th=!th!; _DTH=%_DTH%;  {hue=!hue!}
